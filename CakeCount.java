...
/*
   ...
   Content:
    ...
    解题思路：
            只要数清楚，每个4行中， i或i＋1行的蛋糕数为 (n / 4) * 2 + (n % 4 < 2 ? n % 4 : 2) i＋2或i+3行的蛋糕数为
            ((n - 2) / 4) * 2 + ((n - 2) % 4 < 2 ? (n - 2) % 4 : 2) 然后算清楚一共有多少个完整的4行就行， 当然是共有m / 4
            个。 唯一要注意的就是处理行数除4有余数的情况，把最后可能剩余的1～3 行加进去即可。
...
public class CakeCount {
    ...
    static void CakeNum(int m, int n) {
